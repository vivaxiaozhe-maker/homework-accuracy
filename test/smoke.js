...
/* 前端已拆分为 js/*.js（v1.4.1）：按 html 中 <script src> 顺序逐个读文件拼接（与原单文件字节级一致），再 vm 执行 */
...
(async function main(){
  ...
  ok(html.indexOf('id="login-ver">v1.4.1') !== -1, '登录页版本号升至 v1.4.1');
  ...
  ok(html.indexOf('id="side-foot">演示环境 · 数据暂存本机 · v1.4.1') !== -1
    && documentStub.getElementById('side-foot').textContent === '', 'mock 模式侧栏脚注为「演示环境 · 数据暂存本机 · v1.4.1」（未被覆盖）');
